<?php

// +----------------------------------------------------------------------
// | pay-php-sdk
// +----------------------------------------------------------------------
// | 版权所有 2014~2017 广州楚才信息科技有限公司 [ http://www.cuci.cc ]
// +----------------------------------------------------------------------
// | 开源协议 ( https://mit-license.org )
// +----------------------------------------------------------------------
// | github开源项目：https://github.com/zoujingli/pay-php-sdk
// +----------------------------------------------------------------------

return [
    // 微信支付参数
    'wechat' => [
        // 沙箱模式
        'debug' => env('PAY_WECHAT_DEBUG', false),
        // 应用ID
        'app_id' => env('PAY_WECHAT_APP_ID', 'wxe335431b79068046'),
        // 微信支付商户号
        'mch_id' => env('PAY_WECHAT_MCH_ID', '1300513101'),
        /*
        // 子商户公众账号ID
        'sub_appid'  => '子商户公众账号ID，需要的时候填写',
        // 子商户号
        'sub_mch_id' => '子商户号，需要的时候填写',
         */
        // 微信支付密钥
        'mch_key' => env('PAY_WECHAT_MCH_KEY', 'AGNq9Z6I9xQ7usWT2xPXc76pS9HUvcoq'),
        // 微信证书 cert 文件
        'ssl_cer' => env('PAY_WECHAT_SSL_CER'),
        // 微信证书 key 文件
        'ssl_key' => env('PAY_WECHAT_SSL_KEY'),
        // 缓存目录配置
        'cache_path' => env('PAY_CACHE_PATH'),
        // 支付成功通知地址
        'notify_url' => env('PAY_WECHAT_NOTIFY_URL'),
        // 网页支付回跳地址
        'return_url' => env('PAY_WECHAT_RETURN_URL')
    ],
    // 支付宝支付参数
    'alipay' => [
        // 沙箱模式
        'debug' => env('PAY_ALIPAY_DEBUG', false),
        // 应用ID
        'app_id' => env('PAY_ALIPAY_APP_ID', 2016090900468879),
        // 支付宝公钥(1行填写)
        'public_key' => env('PAY_ALIPAY_PUBLIC_KEY', ''),
        // 支付宝私钥(1行填写)
        'private_key' => env('PAY_ALIPAY_PRIVATE_KEY', ''),
        // 缓存目录配置
        'cache_path' => env('PAY_CACHE_PATH'),
        // 支付成功通知地址
        'notify_url' => env('PAY_ALIPAY_NOTIFY_URL'),
        // 网页支付回跳地址
        'return_url' => env('PAY_ALIPAY_RETURN_URL')
    ]
];
